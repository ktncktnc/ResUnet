    ...
    with torch.no_grad():
        for (idx, data) in enumerate(loader):
            ...
            for i in range(output1.shape[0]):
                ...
                print(masks1.shape)
                print(masks2.shape)
                np.savetxt(os.path.join(masks_save_path, "{filename}_1.txt".format(filename=filename)), masks1, delimiter=',')
                np.savetxt(os.path.join(masks_save_path, "{filename}_2.txt".format(filename=filename)), masks2, delimiter=',')
