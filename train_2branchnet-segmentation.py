...
def main(hpconfig, num_epochs, resume, name, device, training_weight=None):
    ...
    # get data
    # cd
    cd_dataset_train = AlabamaDataset(hpconfig.cd_dset_dir, "train")
    cd_dataset_val = AlabamaDataset(hpconfig.cd_dset_dir, "test")
    # cd_dataset_test = AlabamaDataset(hpconfig.cd_dset_dir, "test")
...
def validation(cd_valid_loader, model, criterion, device, training_weight, write_log=False, logger=None, step=None):
    ...
    for data in cd_valid_loader:
        ...
        s_valid_acc.update(metrics.dice_coeff(outputs, labels), outputs.size(0))
        s_valid_loss.update(loss.data.item(), outputs.size(0))
