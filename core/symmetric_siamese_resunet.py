...
class DependentResUnetMultiDecoder(nn.Module):
    ...
    def create_encoder(self):
        ...
        self.n_decoder_layers = len(self.encoded_channels) - 1
    ...
    def create_siamese_decoder(self):
        ...
        for i in range(1, self.n_decoder_layers):
        ...
        siamese_decoder.append(UpBlockForUNetWithResNet50(
            in_channels=int(self.encoded_channels[0])*2 + 1,
            out_channels=int(self.encoded_channels[0]),
            up_conv_in_channels=int(self.encoded_channels[1]),
            up_conv_out_channels=int(self.encoded_channels[0])
        ))
        ...
        self.siamese_decoder_out = nn.Sequential(
            nn.Conv2d(int(self.encoded_channels[0]), self.cd_o_channel, kernel_size=1, stride=1),
            nn.Sigmoid()
        )
    ...
    def create_segment_decoder(self):
        ...
        segment_decoder.append(UpBlockForUNetWithResNet50(
            in_channels=int(self.encoded_channels[0])*2,
            out_channels=int(self.encoded_channels[0]),
            up_conv_in_channels=int(self.encoded_channels[1]),
            up_conv_out_channels=int(self.encoded_channels[0])
        ))
        ...
        self.segment_decoder_out = nn.Sequential(
            nn.Conv2d(int(self.encoded_channels[0]), self.segment_o_channel, kernel_size=1, stride=1),
            nn.Sigmoid()
        )
    ...
    def siamese_fuse(self, pools_x, pools_y):
        ...
        for i in range(1, len(self.encoded_channels)):
            key = f"layer_{self.n_decoder_layers - i}"
    ...
    def segment_forward(self, x, domain_classify=True, alpha=None, pools=None, return_pool=False):
        ...
        pools['layer_0'] = torch.cat([pools['layer_0'], a[:, 0:1, ...]], 1)
