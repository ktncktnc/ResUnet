        ...
        for (idx, data) in enumerate(loader):
            ...
            for i in range(x.shape[0]):
                ...
                if divide >= dataset.divide*dataset.divide - 1:
                    ...
                    np.savez_compressed(os.path.join(prob_img1_save_path, "{filename}.npz".format(filename=pre_name)), a=full_x_probs)
                    np.savez_compressed(os.path.join(prob_img2_save_path, "{filename}.npz".format(filename=post_name)), a=full_y_probs)
                    # Hungarian algorithm
                    if cd:
